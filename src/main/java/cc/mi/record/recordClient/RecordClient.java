        ...
        try {
	        ...
	        bootstrap.handler(new ChannelInitializer<SocketChannel>() {
	            ...
	            protected void initChannel(SocketChannel ch) throws Exception {
					...
					ch.pipeline().addLast("decoder", new RecordClientDecoder());
					
					ch.pipeline().addLast("clientHandler", new RecordClientHandler());
					
					ch.pipeline().addLast("frameEncoder", new LengthFieldPrepender(4, true));
					ch.pipeline().addLast("encoder", new RecordClientEncoder());
	            }
	        });
